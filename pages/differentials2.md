...
#### --outlinebox right1
Here's a graph of the function $$f(x)= \sqrt{x}.$$
$dx$[](:?dx|number) [set dx](:=setdx=true)
$dy$[](:!dy)
$f(a+dx) - f(a)  = \Delta y$ [](:!delta)
[zoom in](:=zoom=true)
Without using a calculator find $\sqrt{9.1}$ [](:?answer|number)
Linear estimation 
$$y=f(a) + f'(a)(x - a)$$
$$y=\sqrt{9} + \frac{1}{2\sqrt{9}}(x - 9)$$
Now plug in $9.1$ for $x$ and see what $y$ is.  Compare it with your calculator result for $\sqrt{9.1}$.
#### --outlinebox
#### --outlinebox
...
let F = new ProblemFunction(function(x) { return Math.pow(x,0.5); }, '', 4, [xlow,xhigh], []);
let F_id = workspace.addFunction(F);

let xint = new XInterval(workspace.board, 9,12);
xint.x1.setAttribute({name:'9', fixed:true});
xint.x2.setAttribute({name:'9 + dx'});

// get the color scheme for our extra workspace objects
let colors = new SteelTheme();

// tangent stuff 
let d = function(x) { return 0.5 * Math.pow(x,-0.5); }
...
this.depend = function() {
  if (env.zoom == true) {
    smartdown.setVariable('zoom', false);
    workspace.board.zoomIn(9,F.f(9));
  }
...
const formula3 = document.getElementById('MathJax-Element-4-Frame');
